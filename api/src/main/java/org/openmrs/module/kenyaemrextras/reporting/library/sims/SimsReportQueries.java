...
public class SimsReportQueries {
	...
	public static String currentOnARTWithPresumptiveTBQuery() {
		String qry = "select t.patient_id from (\n"
		        ...
		        + "           join kenyaemr_etl.etl_patient_demographics p on p.patient_id=fup.patient_id\n"
		        + "           join kenyaemr_etl.etl_hiv_enrollment e on fup.patient_id=e.patient_id\n"
		        + "           left join kenyaemr_etl.etl_drug_event de on e.patient_id = de.patient_id and de.program='HIV'\n"
		        + "           left outer JOIN\n"
		        + "               (select patient_id,\n"
		        + "                  date(coalesce(NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31'), max(visit_date))) visit_date,\n"
		        + "                  NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31') as effective_disc_date\n"
	...
	public static String pedsCurrentlyOnArtMoreThan12MonthsQuery() {
		String qry = "select t.patient_id from (\n"
		        ...
		        + "                    join kenyaemr_etl.etl_patient_demographics p on p.patient_id=fup.patient_id\n"
		        + "                    join kenyaemr_etl.etl_hiv_enrollment e on fup.patient_id=e.patient_id\n"
		        + "                    left join kenyaemr_etl.etl_drug_event de on e.patient_id = de.patient_id and de.program='HIV' and de.date_started <= date_sub(date(:endDate) , interval 12 MONTH)\n"
		        + "                    left outer JOIN\n"
		        + "                        (select patient_id,\n"
		        + "                           date(coalesce(NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31'), max(visit_date))) visit_date,\n"
		        + "                           NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31') as effective_disc_date\n"
	...
	public static String keyPopsOnARTReceivedHighImpactServices() {
		String qry = "select t.patient_id\n"
		        ...
		        + "               left join (select de.patient_id,min(date(de.date_started)) as date_started, de.program as program, de.discontinued from kenyaemr_etl.etl_drug_event de group by de.patient_id) de\n"
	...
	public static String adultsOnArtScreenedForCervicalCancerQuery() {
		String qry = "select t.patient_id from (\n"
		        ...
		        + "                 timestampdiff(YEAR ,p.dob,date(:endDate)) as age,\n"
		        + "                 p.gender as gender,\n"
		        + "                        coalesce(mid(max(concat(cs.visit_date, cs.pap_smear_screening_result)), 11),mid(max(concat(cs.visit_date, cs.via_vili_screening_result)), 11))   as screening_result,\n"
		        + "                        mid(max(concat(fup.visit_date,fup.tb_status)),11) as tbStatus,\n"
		        + "                        mid(max(concat(fup.visit_date, fup.pregnancy_status)), 11)                       as pregnant_status,\n"
		        + "                        mid(max(concat(fup.visit_date, fup.breastfeeding)), 11)                          as breastfeeding_status,\n"
		        + "                        pp.patient_id                                                                    as pmtct_client,\n"
		        ...
		        + "             (\n"
		        + "                 ((timestampdiff(DAY,date(latest_tca),date(:endDate)) <= 30) and ((date(d.effective_disc_date) > date(:endDate) or date(enroll_date) > date(d.effective_disc_date)) or d.effective_disc_date is null))\n"
		        + "                     and (date(latest_vis_date) >= date(date_discontinued) or date(latest_tca) >= date(date_discontinued) or disc_patient is null) and age >=15 and gender='F' and screening_result in ('Positive','Invasive Cancer')\n"
		        + "                             and (pmtct_client is null and (pregnant_status != 1065 or pregnant_status is null) and\n"
	...
	public static String KpUnSupressedVLQuery() {
		String qry = "select t.patient_id\n"
		        ...
		        + "group by patient_id\n"
		        + "having\n"
		        + "((started_on_drugs is not null and started_on_drugs <> '') and \n"
		        + "(timestampdiff(DAY,date(latest_tca),date(:endDate)) <= 30 and ((date(d.effective_disc_date) > date(:endDate) or date(enroll_date) > date(d.effective_disc_date)) or d.effective_disc_date is null))\n"
		        + "and (date(latest_vis_date) >= date(date_discontinued) or date(latest_tca) >= date(date_discontinued) or disc_patient is null) and age >=15\n"
		        + ")order by de.date_started desc) t limit 10;";
		return qry;
	...
	public static String txCurrKPsCacxPositive() {
		String query = "select t.patient_id\n"
		        ...
		        + "                       left join (select s.patient_id, coalesce(s.pap_smear_screening_result, s.via_vili_screening_result) as screening_result\n"
		        + "                                  from kenyaemr_etl.etl_cervical_cancer_screening s\n"
		        + "                                  where s.visit_date between date_sub(date(:endDate), INTERVAL 90 DAY)\n"
		        + "                                      and date(:endDate)\n"
		        + "                                    and s.pap_smear_screening_result in ('Positive','Invasive Cancer') or s.via_vili_screening_result = 'Positive') s on e.patient_id = s.patient_id\n"
		        + "                       left outer JOIN\n"
		        + "                   (select patient_id,\n"
		        + "                       date(coalesce(NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31'), max(visit_date))) visit_date,\n"
		        + "                       NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31') as effective_disc_date\n"

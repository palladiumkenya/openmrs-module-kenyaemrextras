...
import org.openmrs.module.kenyaemrextras.reporting.data.definition.sims.SimsKPsHighImpactServicesDataDefinition;
 ...
 */
@Handler(supports = SimsKPsHighImpactServicesDataDefinition.class, order = 50)
public class SimsKPsHighImpactServicesDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select d.patient_id,\n"
		        + "               if(cacx_client is not null or vmmc_client is not null or sti_client is not null or cv.sti_cv_client is not null, 'Y',\n"
		        ...
		        + "                            where cacx.cervical_cancer = 'Yes' and (cacx.via_vili_screening_result in ('Positive', 'Suspicious for Cancer') or cacx.pap_smear_screening_result in ('High grade lesion','Invasive Cancer','Atypical squamous cells(ASC-US/ASC-H)','AGUS'))\n"
		        ...
		        + "                 left join (select cv.client_id as sti_cv_client, cv.visit_date, cv.sti_screened\n"
		        + "                            from kenyaemr_etl.etl_clinical_visit cv\n"
		        + "                            where cv.sti_screened = 'Y' and cv.sti_results = 'Positive' and (cv.sti_referred = 'Yes' or cv.sti_treated = 'Yes')\n"
		        + "                              and date(cv.visit_date) between DATE_SUB(DATE(:endDate), INTERVAL 3 MONTH) and date(:endDate)) cv\n"
		        + "                           on d.patient_id = cv.sti_cv_client;";

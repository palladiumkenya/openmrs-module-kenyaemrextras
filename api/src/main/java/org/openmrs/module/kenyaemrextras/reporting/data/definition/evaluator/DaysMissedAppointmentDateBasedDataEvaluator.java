...
public class DaysMissedAppointmentDateBasedDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select t.patient_id,ifnull(days_missed,'Not returned')\n"
		        ...
		        + "                           and timestampdiff(DAY,date(latest_tca),date(:endDate)) >= 0 and ((date(d.effective_disc_date) > date(:endDate) or date(enroll_date) > date(d.effective_disc_date)) or d.effective_disc_date is null)\n"

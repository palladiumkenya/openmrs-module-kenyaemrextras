...
import org.openmrs.module.kenyaemrextras.reporting.data.definition.sims.SimsPregBFEACAfterUnsuppressedVLStatusDataDefinition;
 ...
 * non-suppression
 */
@Handler(supports = SimsPregBFEACAfterUnsuppressedVLStatusDataDefinition.class, order = 50)
public class SimsPregBFEACAfterUnsuppressedVLStatusDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select a.patient_id,if(vl_result > 1000 and vl_result_date < eac_date,'Y','N') as valid_eac\n"
		        ...
		        + "               inner join (select e.patient_id, max(e.visit_date) as ea_date\n"
		        + "                           from kenyaemr_etl.etl_enhanced_adherence e\n"
		        + "                           where e.visit_date <= date(:endDate)\n"
		        + "                           group by e.patient_id) e on b.patient_id = e.patient_id\n"
		        + "      group by patient_id\n" + "      having vl_date between\n"
		        + "          date_sub(date(:endDate), interval 12 MONTH) and date(:endDate)\n"
		        + "      ) a where a.vl_result >= 1000\n" + "              and a.eac_date > vl_result_date\n"
		        + "            and a.order_reason != 843\n" + "group by patient_id;";

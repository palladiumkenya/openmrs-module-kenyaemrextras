...
import org.openmrs.module.kenyaemrextras.reporting.data.definition.mortalityAuditTool.HeiMotherHaartStatusDataDefinition;
...
/**
 * Evaluates Haart Status
 */
@Handler(supports = HeiMotherHaartStatusDataDefinition.class, order = 50)
public class HeiMotherHaartStatusDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select distinct r.person_a,\n"
		        + "      if(coalesce(date(de.date_started),date(v.date_given_haart),date(ld.visit_date),date(pnc.visit_date)) >= date(mch.visit_date),'Newly initiated ART','Already on ART')     as mother_haart_status\n"

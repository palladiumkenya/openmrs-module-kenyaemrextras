...
/**
 * Evaluator for DeceasedTBPatientCohortDefinition
 */
@Handler(supports = { DeceasedTBPatientCohortDefinition.class })
public class DeceasedTBPatientCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		DeceasedTBPatientCohortDefinition definition = (DeceasedTBPatientCohortDefinition) cohortDefinition;
		...
		String qry = "select d.patient_id\n"
		        ...
		        + "where coalesce(date(d.effective_discontinuation_date), date(d.visit_date),\n"
		        + "               date(d.date_died)) between date(:startDate) and date(:endDate)\n"
		        + "  and d.discontinuation_reason = 160034\n"
		        + "  and d.program_name = 'TB'\n"
		        + "  and e.patient_id is null\n"
		        + "  and (\n"
		        + "        (tb.tb_patient is not null\n"
		        + "            and timestampdiff(MONTH, tb.tb_enrollment_date,\n"
		        + "                              coalesce(date(d.effective_discontinuation_date), date(d.visit_date))) < 10)\n"

...
public class TxcurrWRAWithNoChildrenContactsCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		String qry = "select t.patient_id\n"
		        ...
		        + "               left join (select c.patient_id, c.patient_related_to\n"
		        + "                          from kenyaemr_etl.etl_patient_contact c join kenyaemr_etl.etl_patient_demographics d on c.patient_id = d.patient_id\n"
		        + "                          where timestampdiff(YEAR, date(d.DOB), date(:endDate)) < 15\n"
		        + "                            and date(c.date_created) <= date(:endDate)\n"
		        + "                            and c.relationship_type = 3) c on fup.patient_id = c.patient_related_to\n"

...
/**
 * Evaluator for Active on ART Peds
 */
@Handler(supports = { ARTPedsQualityOfCareCohortDefinition.class })
public class ARTPedsQualityOfServiceCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		ARTPedsQualityOfCareCohortDefinition definition = (ARTPedsQualityOfCareCohortDefinition) cohortDefinition;
		...
		String qry = "select t.patient_id\n"
		        ...
		        + "          left outer JOIN\n"
		        + "          (select patient_id,\n"
		        + "               date(coalesce(NULLIF(LEAST(COALESCE(max(date(trf_out_verification_date)),'9999-12-31'),COALESCE(max(date(effective_discontinuation_date)),'9999-12-31')),'9999-12-31'), max(visit_date))) visit_date,\n"
		        ...
		        + "          ))) order by latest_vis_date desc limit 10) t;";

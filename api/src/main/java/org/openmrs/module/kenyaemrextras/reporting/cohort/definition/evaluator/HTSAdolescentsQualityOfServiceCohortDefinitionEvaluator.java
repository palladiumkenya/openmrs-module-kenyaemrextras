...
/**
 * Evaluator for HTS adolescents
 */
@Handler(supports = { HTSAdolescentsQualityOfCareCohortDefinition.class })
public class HTSAdolescentsQualityOfServiceCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
	...
	public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) throws EvaluationException {
		...
		HTSAdolescentsQualityOfCareCohortDefinition definition = (HTSAdolescentsQualityOfCareCohortDefinition) cohortDefinition;
		...
		String qry = "SELECT t.patient_id from kenyaemr_etl.etl_hts_test t inner join kenyaemr_etl.etl_patient_demographics p on p.patient_id=t.patient_id and p.voided = 0 where t.voided = 0 and date(t.visit_date) BETWEEN date(:startDate) AND date(:endDate)\n"
		        + "and timestampdiff(YEAR ,p.dob, date (:endDate)) < 15 between 10 and 19 order by t.visit_date desc limit 10;";

...
public class SimsTxCurrKpsCacxTreatmentDocumentationStatusDataEvaluator implements PersonDataEvaluator {
	...
	public EvaluatedPersonData evaluate(PersonDataDefinition definition, EvaluationContext context)
		...
		String qry = "select a.patient_id,\n"
		        ...
		        + "                  or (a.scrn_result in ('Positive','High grade lesion','Invasive Cancer','Atypical squamous cells(ASC-US/ASC-H)','AGUS','Suspicious for Cancer') and\n"
		        ...
		        + "                                                   'Hysterectomy', 'Referred for cancer treatment','Refer for colposcopy','Refer for biopsy','Perform biopsy and/or refer for further management'))), 'Y', 'N'))\n"
		        ...
		        + "                                  coalesce(mid(max(concat(s.visit_date, s.pap_smear_screening_result)), 11),mid(max(concat(s.visit_date, s.via_vili_screening_result)), 11))       as scrn_result,\n"
		        + "                                  coalesce(mid(max(concat(s.visit_date, s.via_vili_treatment_method)), 11),mid(max(concat(s.visit_date, s.pap_smear_treatment_method)), 11))       as scrn_treatment_method\n"
		        ...
		        + "                          on e.patient_id = v.client_id group by e.patient_id\n"
